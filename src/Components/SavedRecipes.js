                ...
                {savedRecipes.length > 0 ? savedRecipes.map((e, i) =>
                    <div key={i} className="mb-2">
                        <a href={e.url} className="mr-2">
                            <Button style={{ maxWidth: "80%", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap" }}>
                                {e.title}
                            </Button>
                        </a>
                        <Button><FontAwesomeIcon icon={faMinus} onClick={() => manageSavedRecipes(e.id)} /></Button>
                    </div>)
